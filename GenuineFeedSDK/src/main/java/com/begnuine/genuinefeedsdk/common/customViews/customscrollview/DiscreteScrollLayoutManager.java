package com.begnuine.genuinefeedsdk.common.customViews.customscrollview;
...
import com.begnuine.genuinefeedsdk.common.customViews.customscrollview.transform.DiscreteScrollItemTransformer;
...
public class DiscreteScrollLayoutManager extends RecyclerView.LayoutManager {
    ...
    protected void initChildDimensions(RecyclerView.Recycler recycler) {
        ...
        Utility.showLog("Time", "Scroll change current " + scrollToChangeCurrent);
    ...
    protected int scrollBy(int amount, RecyclerView.Recycler recycler) {
        ...
        Utility.showLog("Amount", "Amount " + amount);
        Direction direction = Direction.fromDelta(amount);
        int leftToScroll = calculateAllowedScrollIn(direction);
        Utility.showLog("Time", "Left to scroll " + leftToScroll);
        if (leftToScroll <= 0) {
            return 0;
        }

        int delta = direction.applyTo(Math.min(leftToScroll, Math.abs(amount)));
        Utility.showLog("Time", "Delta " + delta);
        scrolled += delta;
        if (pendingScroll != 0) {
            pendingScroll -= delta;
        }

        Utility.showLog("Time", "scrolled " + scrolled);
        Utility.showLog("Time", "Pending Scroll " + pendingScroll);
    ...
    private void notifyScroll() {
        ...
        Utility.showLog("Time", position + " Pending Scroll : " + pendingScroll + " scrolled : " + scrolled);
